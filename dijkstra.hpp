...
struct dijkstraComparator {
    bool operator()(const int& idx_a, const int& idx_b) const {
        return dijkstra_nodes[idx_a].g > dijkstra_nodes[idx_b].g;
    }
};
...
void dijkstra_search(double* map, int x_size, int y_size, int start_x, int start_y, int goal_x, int goal_y) {
